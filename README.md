...
<!-- generated:hook -->
Our estimator's Brier loss was 22.6% worse than the market's. We know
...
<!-- generated:headline -->
**984 forecasts · 436 on-chain anchors · 962 public proofs · 0 undisclosed
production roots · Brier skill −0.226 across 7 model versions at N=968.** The
...
`0.2916`; the market's is `0.2378`. Skill is `−0.2265`, with a deterministic
1,000-resample 95% interval from `−0.2878` to `−0.1641` at `N=968`
(`dashboard/app/forecast-data.json`, key `resolve_score.all_evaluated_windows`).
That is a loss. We display it.
<!-- /generated:skill-mixed -->

<!-- generated:skill-gate -->
The mixed-history risk-gate subset is `−0.0392` at `N=217`, with an interval from
`−0.0753` to `−0.0018`
...
windows, skill is `−0.1974` at `N=751`, with a 95% interval from `−0.2698` to
`−0.1350`. Its risk-gate subset is `−0.0440` at `N=165`, with an interval from
`−0.0808` to `−0.0066`
...
At watermark block `474858589`, the audit sees 437 on-chain
roots, 436 disclosed roots and zero hidden roots
