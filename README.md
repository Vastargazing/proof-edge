...
<!-- generated:hook -->
My estimator's Brier loss was 26.4% worse than the market's. I know
...
<!-- generated:headline -->
**3465 forecasts · 1504 disclosed roots · 3444 public proofs · 0 undisclosed
production roots · Brier skill −0.264 across 7 model versions at N=3450.** The
...
15 newer forecasts were still waiting for resolution in the published snapshot.
...
`0.2994`; the market's is `0.2368`. Skill is `−0.2643`, with a deterministic
1,000-resample 95% interval from `−0.2975` to `−0.2312` at `N=3450`
(`dashboard/app/forecast-data.json`, key `resolve_score.all_evaluated_windows`).
That is a loss. I display it.
<!-- /generated:skill-mixed -->

<!-- generated:skill-gate -->
The mixed-history risk-gate subset is `−0.0238` at `N=664`, with an interval from
`−0.0444` to `−0.0024`
...
windows, skill is `−0.2600` at `N=3233`, with a 95% interval from `−0.2932` to
`−0.2277`. Its risk-gate subset is `−0.0239` at `N=612`, with an interval from
`−0.0452` to `−0.0022`
...
At watermark block `478097316`, the audit sees 1505 on-chain
anchor events. The ledger accounts for 1504 unique roots; zero
