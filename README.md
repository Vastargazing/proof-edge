...
<!-- generated:hook -->
Our estimator's Brier loss was 26.2% worse than the market's. We know
...
<!-- generated:headline -->
**402 forecasts · 182 on-chain anchors · 384 public proofs · 0 undisclosed
production roots · Brier skill −0.262 across 7 model versions at N=390.** The
...
`0.3056`; the market's is `0.2421`. Skill is `−0.2624`, with a deterministic
1,000-resample 95% interval from `−0.3686` to `−0.1580` at `N=390`
(`dashboard/app/forecast-data.json`, key `resolve_score.all_evaluated_windows`).
That is a loss. We display it.
<!-- /generated:skill-mixed -->

<!-- generated:skill-gate -->
The mixed-history risk-gate subset is `−0.0189` at `N=79`, with an interval from
`−0.0849` to `0.0458`
...
windows, skill is `−0.1847` at `N=173`, with a 95% interval from `−0.3379` to
`−0.0322`. Its risk-gate subset is `−0.0113` at `N=27`, with an interval from
`−0.1160` to `0.0970`
...
At watermark block `473994795`, the audit sees 182 on-chain
roots, 182 disclosed roots and zero hidden roots
