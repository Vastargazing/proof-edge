...
<!-- generated:hook -->
Our estimator's Brier loss was 27.6% worse than the market's. We know
...
<!-- generated:headline -->
**2822 forecasts · 1239 on-chain anchors · 2801 public proofs · 0 undisclosed
production roots · Brier skill −0.276 across 7 model versions at N=2807.** The
skill figure is the mixed historical total, not the result of the current model
version; its two samples are reported separately below. 15 newer forecasts were still waiting for resolution in the published snapshot.
...
`0.3033`; the market's is `0.2377`. Skill is `−0.2763`, with a deterministic
1,000-resample 95% interval from `−0.3109` to `−0.2422` at `N=2807`
(`dashboard/app/forecast-data.json`, key `resolve_score.all_evaluated_windows`).
That is a loss. We display it.
<!-- /generated:skill-mixed -->

<!-- generated:skill-gate -->
The mixed-history risk-gate subset is `−0.0299` at `N=552`, with an interval from
`−0.0551` to `−0.0054`
...
windows, skill is `−0.2719` at `N=2590`, with a 95% interval from `−0.3061` to
`−0.2320`. Its risk-gate subset is `−0.0306` at `N=500`, with an interval from
`−0.0545` to `−0.0059`
...
At watermark block `476801880`, the audit sees 1239 on-chain
roots, 1238 disclosed roots and zero hidden roots
