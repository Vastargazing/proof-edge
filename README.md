...
<!-- generated:hook -->
Our estimator's Brier loss was 23.5% worse than the market's. We know
...
<!-- generated:headline -->
**950 forecasts · 419 on-chain anchors · 928 public proofs · 0 undisclosed
production roots · Brier skill −0.235 across 7 model versions at N=934.** The
...
`0.2944`; the market's is `0.2384`. Skill is `−0.2349`, with a deterministic
1,000-resample 95% interval from `−0.2951` to `−0.1725` at `N=934`
(`dashboard/app/forecast-data.json`, key `resolve_score.all_evaluated_windows`).
That is a loss. We display it.
<!-- /generated:skill-mixed -->

<!-- generated:skill-gate -->
The mixed-history risk-gate subset is `−0.0336` at `N=209`, with an interval from
`−0.0683` to `0.0030`
...
windows, skill is `−0.2070` at `N=717`, with a 95% interval from `−0.2753` to
`−0.1357`. Its risk-gate subset is `−0.0368` at `N=157`, with an interval from
`−0.0759` to `0.0034`
...
At watermark block `474822695`, the audit sees 420 on-chain
roots, 419 disclosed roots and zero hidden roots
