...
<!-- generated:hook -->
Our estimator's Brier loss was 23.0% worse than the market's. We know
...
<!-- generated:headline -->
**640 forecasts · 289 on-chain anchors · 618 public proofs · 0 undisclosed
production roots · Brier skill −0.230 across 7 model versions at N=624.** The
...
`0.2913`; the market's is `0.2368`. Skill is `−0.2300`, with a deterministic
1,000-resample 95% interval from `−0.3084` to `−0.1546` at `N=624`
(`dashboard/app/forecast-data.json`, key `resolve_score.all_evaluated_windows`).
That is a loss. We display it.
<!-- /generated:skill-mixed -->

<!-- generated:skill-gate -->
The mixed-history risk-gate subset is `−0.0441` at `N=136`, with an interval from
`−0.0891` to `0.0070`
...
windows, skill is `−0.1782` at `N=407`, with a 95% interval from `−0.2721` to
`−0.0855`. Its risk-gate subset is `−0.0559` at `N=84`, with an interval from
`−0.1131` to `−0.0003`
...
At watermark block `474498665`, the audit sees 290 on-chain
roots, 289 disclosed roots and zero hidden roots
