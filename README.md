...
<!-- generated:hook -->
Our estimator's Brier loss was 22.5% worse than the market's. We know
...
<!-- generated:headline -->
**844 forecasts · 377 on-chain anchors · 823 public proofs · 0 undisclosed
production roots · Brier skill −0.225 across 7 model versions at N=829.** The
...
`0.2935`; the market's is `0.2396`. Skill is `−0.2253`, with a deterministic
1,000-resample 95% interval from `−0.2942` to `−0.1588` at `N=829`
(`dashboard/app/forecast-data.json`, key `resolve_score.all_evaluated_windows`).
That is a loss. We display it.
<!-- /generated:skill-mixed -->

<!-- generated:skill-gate -->
The mixed-history risk-gate subset is `−0.0420` at `N=182`, with an interval from
`−0.0823` to `−0.0040`
...
windows, skill is `−0.1896` at `N=612`, with a 95% interval from `−0.2649` to
`−0.1130`. Its risk-gate subset is `−0.0490` at `N=130`, with an interval from
`−0.0905` to `−0.0027`
...
At watermark block `474714619`, the audit sees 378 on-chain
roots, 377 disclosed roots and zero hidden roots
