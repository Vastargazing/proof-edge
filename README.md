...
<!-- generated:hook -->
Our estimator's Brier loss was 25.8% worse than the market's. We know
...
<!-- generated:headline -->
**606 forecasts · 276 on-chain anchors · 584 public proofs · 0 undisclosed
production roots · Brier skill −0.258 across 7 model versions at N=590.** The
...
`0.2989`; the market's is `0.2375`. Skill is `−0.2584`, with a deterministic
1,000-resample 95% interval from `−0.3429` to `−0.1804` at `N=590`
(`dashboard/app/forecast-data.json`, key `resolve_score.all_evaluated_windows`).
That is a loss. We display it.
<!-- /generated:skill-mixed -->

<!-- generated:skill-gate -->
The mixed-history risk-gate subset is `−0.0409` at `N=129`, with an interval from
`−0.0862` to `0.0100`
...
windows, skill is `−0.2181` at `N=373`, with a 95% interval from `−0.3222` to
`−0.1113`. Its risk-gate subset is `−0.0524` at `N=77`, with an interval from
`−0.1117` to `0.0111`
...
At watermark block `474462778`, the audit sees 277 on-chain
roots, 276 disclosed roots and zero hidden roots
