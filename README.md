...
<!-- generated:hook -->
Our estimator's Brier loss was 26.4% worse than the market's. We know
...
<!-- generated:headline -->
**388 forecasts · 175 on-chain anchors · 370 public proofs · 0 undisclosed
production roots · Brier skill −0.264 across 7 model versions at N=376.** The
...
`0.3072`; the market's is `0.2431`. Skill is `−0.2638`, with a deterministic
1,000-resample 95% interval from `−0.3669` to `−0.1654` at `N=376`
(`dashboard/app/forecast-data.json`, key `resolve_score.all_evaluated_windows`).
That is a loss. We display it.
<!-- /generated:skill-mixed -->

<!-- generated:skill-gate -->
The mixed-history risk-gate subset is `−0.0197` at `N=75`, with an interval from
`−0.0901` to `0.0477`
...
windows, skill is `−0.1822` at `N=159`, with a 95% interval from `−0.3500` to
`−0.0183`. Its risk-gate subset is `−0.0121` at `N=23`, with an interval from
`−0.1305` to `0.1154`
...
At watermark block `473814844`, the audit sees 175 on-chain
roots, 175 disclosed roots and zero hidden roots
