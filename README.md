...
<!-- generated:hook -->
Our estimator's Brier loss was 23.4% worse than the market's. We know
...
<!-- generated:headline -->
**707 forecasts · 316 on-chain anchors · 685 public proofs · 0 undisclosed
production roots · Brier skill −0.234 across 7 model versions at N=691.** The
...
`0.2943`; the market's is `0.2385`. Skill is `−0.2339`, with a deterministic
1,000-resample 95% interval from `−0.3077` to `−0.1578` at `N=691`
(`dashboard/app/forecast-data.json`, key `resolve_score.all_evaluated_windows`).
That is a loss. We display it.
<!-- /generated:skill-mixed -->

<!-- generated:skill-gate -->
The mixed-history risk-gate subset is `−0.0406` at `N=143`, with an interval from
`−0.0839` to `0.0016`
...
windows, skill is `−0.1915` at `N=474`, with a 95% interval from `−0.2848` to
`−0.1019`. Its risk-gate subset is `−0.0500` at `N=91`, with an interval from
`−0.1030` to `0.0066`
...
At watermark block `474570811`, the audit sees 317 on-chain
roots, 316 disclosed roots and zero hidden roots
