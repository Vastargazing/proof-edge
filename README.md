...
<!-- generated:hook -->
Our estimator's Brier loss was 22.9% worse than the market's. We know
...
<!-- generated:headline -->
**674 forecasts · 303 on-chain anchors · 652 public proofs · 0 undisclosed
production roots · Brier skill −0.229 across 7 model versions at N=658.** The
...
`0.2907`; the market's is `0.2364`. Skill is `−0.2295`, with a deterministic
1,000-resample 95% interval from `−0.3024` to `−0.1476` at `N=658`
(`dashboard/app/forecast-data.json`, key `resolve_score.all_evaluated_windows`).
That is a loss. We display it.
<!-- /generated:skill-mixed -->

<!-- generated:skill-gate -->
The mixed-history risk-gate subset is `−0.0408` at `N=142`, with an interval from
`−0.0853` to `0.0018`
...
windows, skill is `−0.1812` at `N=441`, with a 95% interval from `−0.2705` to
`−0.0956`. Its risk-gate subset is `−0.0502` at `N=90`, with an interval from
`−0.1033` to `0.0049`
...
At watermark block `474534781`, the audit sees 304 on-chain
roots, 303 disclosed roots and zero hidden roots
