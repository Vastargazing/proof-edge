...
<!-- generated:hook -->
Our estimator's Brier loss was 27.8% worse than the market's. We know
...
<!-- generated:headline -->
**2952 forecasts · 1294 on-chain anchors · 2940 public proofs · 0 undisclosed
production roots · Brier skill −0.278 across 7 model versions at N=2946.** The
skill figure is the mixed historical total, not the result of the current model
version; its two samples are reported separately below. 6 newer forecasts were still waiting for resolution in the published snapshot.
...
`0.3053`; the market's is `0.2388`. Skill is `−0.2781`, with a deterministic
1,000-resample 95% interval from `−0.3142` to `−0.2425` at `N=2946`
(`dashboard/app/forecast-data.json`, key `resolve_score.all_evaluated_windows`).
That is a loss. We display it.
<!-- /generated:skill-mixed -->

<!-- generated:skill-gate -->
The mixed-history risk-gate subset is `−0.0274` at `N=577`, with an interval from
`−0.0506` to `−0.0059`
...
windows, skill is `−0.2741` at `N=2729`, with a 95% interval from `−0.3127` to
`−0.2411`. Its risk-gate subset is `−0.0278` at `N=525`, with an interval from
`−0.0516` to `−0.0049`
...
At watermark block `477502263`, the audit sees 1295 on-chain
roots, 1294 disclosed roots and zero hidden roots
