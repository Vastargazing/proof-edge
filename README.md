...
<!-- generated:hook -->
My estimator's Brier loss was 27.3% worse than the market's. I know
...
<!-- generated:headline -->
**3912 forecasts · 1695 disclosed roots · 3893 public proofs · 0 undisclosed
production roots · Brier skill −0.273 across 7 model versions at N=3899.** The
...
13 newer forecasts were still waiting for resolution in the published snapshot.
...
`0.3015`; the market's is `0.2368`. Skill is `−0.2730`, with a deterministic
1,000-resample 95% interval from `−0.3043` to `−0.2440` at `N=3899`
(`dashboard/app/forecast-data.json`, key `resolve_score.all_evaluated_windows`).
That is a loss. I display it.
<!-- /generated:skill-mixed -->

<!-- generated:skill-gate -->
The mixed-history risk-gate subset is `−0.0212` at `N=723`, with an interval from
`−0.0414` to `−0.0018`
...
windows, skill is `−0.2697` at `N=3682`, with a 95% interval from `−0.3015` to
`−0.2368`. Its risk-gate subset is `−0.0211` at `N=671`, with an interval from
`−0.0425` to `0.0015`
...
At watermark block `478565445`, the audit sees 1696 on-chain
anchor events. The ledger accounts for 1695 unique roots; zero
