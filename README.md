...
<!-- generated:hook -->
Our estimator's Brier loss was 22.8% worse than the market's. We know
...
<!-- generated:headline -->
**811 forecasts · 362 on-chain anchors · 789 public proofs · 0 undisclosed
production roots · Brier skill −0.228 across 7 model versions at N=795.** The
...
`0.2942`; the market's is `0.2396`. Skill is `−0.2277`, with a deterministic
1,000-resample 95% interval from `−0.2938` to `−0.1612` at `N=795`
(`dashboard/app/forecast-data.json`, key `resolve_score.all_evaluated_windows`).
That is a loss. We display it.
<!-- /generated:skill-mixed -->

<!-- generated:skill-gate -->
The mixed-history risk-gate subset is `−0.0372` at `N=174`, with an interval from
`−0.0778` to `0.0049`
...
windows, skill is `−0.1909` at `N=578`, with a 95% interval from `−0.2650` to
`−0.1157`. Its risk-gate subset is `−0.0427` at `N=122`, with an interval from
`−0.0862` to `0.0019`
...
At watermark block `474678731`, the audit sees 363 on-chain
roots, 362 disclosed roots and zero hidden roots
