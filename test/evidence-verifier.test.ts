...
import { verifyPublishedEvidence, type ChainAnchorReader } from "../src/evidence-verifier.js";
...
test("negative step 1: one-digit p_agent mutation returns FAIL at canonical preimage", async () => {
  ...
  const result = await verifyPublishedEvidence(tampered, readFixtureAnchor);
  printResult(result);
...
test("negative step 4: synthetic late anchor returns NOT PROVABLE using nanoseconds", async () => {
  ...
  assert.deepEqual(result.steps.map((step) => step.status), ["PASS", "PASS", "PASS", "NOT PROVABLE"]);
  assert.equal(result.steps.at(-1)?.step, 4);
  assert.match(result.steps.at(-1)?.message ?? "", /anchor_ns 3000000000 is not before expiry_ns 2000000000/);
});
